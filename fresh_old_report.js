...
// Removed: const viewEntriesBtn = document.getElementById('view-entries-btn');
...
// Removed: detailed entry DOM elements
...
function populateFilters() {
    ...
    // --- START Fixed default and disable Company Filter ---
    const fixedCompany = 'VANCHINAD FINANCE LTD';
    
    // Set the default value. If the option exists, it will be selected.
    companySelect.value = fixedCompany;
    
    // Disable the filter to prevent user changes
    companySelect.disabled = true;
    
    // --- END Fixed default and disable Company Filter ---

    // Existing logic for Branch
...
function generateReport() {
    // Only filter for the selected date range for Overall and Company-wise data
    const filteredDataForOverallAndCompany = getFilteredData(false);
    
    // Use all months in the data set for Monthly Trends, but still filtered by Company/Branch
    const filteredDataForMonthlyTrends = getFilteredData(true).filter(row => {
        const selectedCompany = companySelect.value;
        const selectedBranch = branchSelect.value;
        const companyColIndex = headers.indexOf('COMPANY NAME');
        const branchColIndex = headers.indexOf('BRANCH');
    ...
    // Removed: detailedEntriesContainer.style.display = 'none';
    ...
    // --- 2. Monthly Trends (using all-month-filtered data) ---
    const monthlyData = {};
...
// Removed: function viewDetailedEntries() {}

// --- First-level drilldown: Fresh Customers by Staff ---
function showFreshCustomersByStaffDrilldown() {
    staffPerformanceModal.style.display = 'none';
    customerDetailsModal.style.display = 'none';
    freshCustomerModal.style.display = 'flex'; // Use flex for centering via CSS
    ...
    sortedStaff.forEach(([staffName, data]) => {
        ...
        // The cell containing the count is the drilldown point
        freshCustomerCountCell.classList.add('drilldown-cell'); 
...
function showStaffPerformanceDrilldown() {
    ...
    staffPerformanceModal.style.display = 'flex'; // Use flex for centering via CSS
}
...
function showCustomerDetailsDrilldown(staffName) {
    ...
    customerDetailsModal.style.display = 'flex'; // Use flex for centering via CSS
}
...
// Removed: viewEntriesBtn.addEventListener('click', viewDetailedEntries);

// Drilldown listeners on the overall totals
