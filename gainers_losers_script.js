...
const companyShortNames = {
    'SML FINANCE LTD': 'SML',
    'BRD FINANCE LTD': 'BRD',
    'VANCHINAD FINANCE LTD': 'VFL', // <-- Target Company
    'SANGEETH NIDHI LTD': 'SNL'
};

const DEFAULT_COMPANY = 'VANCHINAD FINANCE LTD'; // <-- New Constant for the locked company

// --- Global Data Storage ---
let allData = []; // Stores all parsed CSV rows
let headers = []; // Stores CSV headers
let allCompanyNames = []; // Stores all unique company names for filter search (Now redundant but kept for minimum change)
...
// let companySearchInput; // REMOVED
let companySelect;
let monthSelect;
// NEW: Date Range Inputs
let startDateInput;
let endDateInput;
// let showResignedCheckbox; // REMOVED
// let showDirectCheckbox; // REMOVED
let reportSection;
...
async function init() {
    try {
        ...
        // showResignedCheckbox and showDirectCheckbox removed
        ...
        // REMOVED: Event listeners for showResignedCheckbox and showDirectCheckbox
...
function populateFilters() {
    // Logic remains the same (only populating month/date filters)
...
function getFilteredData() {
    // Company is now fixed
    const selectedCompany = DEFAULT_COMPANY;
    ...
    return allData.filter(row => {
        ...
        // 1. Filter by company (Locked to DEFAULT_COMPANY)
        ...
        // 3. Filter by staff status (REMOVED - all staff types now included)
        // The row is included if it passed the company and date filters.
        return true; 
    });
}

// --- Generate Report (no changes needed inside this function) ---
function generateReport() {
    ...
    filteredData.forEach(row => {
        ...
        // Keep status extraction for styling/data purposes
